...
/* Q1.Answer 
Number of rows in each table are  : - 
...
/* Total No. of columns in each table of the schema - 
...
-- for 2nd part of the question (month wise trend)
...
-- Drama Genre has highest number of movies.
...
/*
the above code groups the movie_id and count the genre count
-- output is:
	movie_id   count_of_genre
	tt0012494	  3
	tt0038733	  3
	tt0060908	  2
	tt0069049	  1
...
/*
then from above CTE, we count those movie_id's who's genre count is 1
*/
...
from movie as mov
		...
		ratings as rat
		on mov.id = rat.movie_id
	where languages like '%italian%' or languages like '%german%' 
Group by languages ; 
...
+-------------------+-------------------+---------------------+*/
-- Type your code below:
with production_comnany_rank AS (
...
--------------------------------------------------------------------------------------------*/
-- Type your code below:
select title as thriller_movie_title , avg_rating ,
			CASE 
				when avg_rating > 8 then 'Superhit Movies'
                when avg_rating between 7 and 8 then 'Hit Movies'
                when avg_rating between 5 and 7 then 'One-time-watch movies'
                ELSE 'Flop movies'
			END AS thriller_movies_classification 
FROM movie as mov
INNER JOIN ratings as rat
on mov.id = rat.movie_id
Inner join genre as gen
...
window w1 as (order by avg_duration ROWS unbounded preceding) ,
 w2 as ( order by avg_duration rows 10 preceding) ;
